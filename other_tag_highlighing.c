...
static gboolean is_tag_empty(gchar *tagName)
{
    ...
    for(unsigned int i=0; i<(sizeof(emptyTags)/sizeof(emptyTags[0])); i++)
    {
...
static void find_matching_opening_tag(ScintillaObject *sci, gchar *tagName, gint openingBracket)
{
    ...
    for(gint pos=openingBracket; pos>0; pos--)
    {
        ...
        gint matchingOpeningBracket = find_bracket(sci, pos, lineStart, '<', '\0', FALSE);
        gint matchingClosingBracket = find_bracket(sci, pos, lineStart, '>', '\0', FALSE);
        ...
        /* Speed up search: if find_bracket returns -1, that means start of line
...
static void find_matching_closing_tag(ScintillaObject *sci, gchar *tagName, gint closingBracket)
{
    ...
    for(gint pos=closingBracket; pos<endOfDocument; pos++)
    {
        ...
        gint matchingOpeningBracket = find_bracket(sci, pos, lineEnd, '<', '\0', TRUE);
        gint matchingClosingBracket = find_bracket(sci, pos, lineEnd, '>', '\0', TRUE);
